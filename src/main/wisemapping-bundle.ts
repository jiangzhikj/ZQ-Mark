...
const VERSION_FILE = '.zq-wisemapping-version'

export function getUserWisemappingRoot(): string {
  return join(app.getPath('userData'), 'plugins', 'wisemapping')
}

/** 解析顺序：userData 已安装 → 打包内置 → 开发仓库 resources */
export function resolveWisemappingIndexPath(): string | null {
  const userIndex = join(getUserWisemappingRoot(), 'index.html')
  if (existsSync(userIndex)) {
    return userIndex
  }

  if (app.isPackaged) {
    const bundled = join(process.resourcesPath, 'wisemapping', 'index.html')
    if (existsSync(bundled)) {
      return bundled
    }
  } else {
    const devIndex = join(__dirname, '../../resources/wisemapping/index.html')
...
export function getWisemappingIndexAssetUrl(): string | null {
  const p = resolveWisemappingIndexPath()
  return p ? localPathToAssetUrl(p) : null
}

async function readInstalledVersion(): Promise<string | undefined> {
  const vf = join(getUserWisemappingRoot(), VERSION_FILE)
...
export async function getWisemappingBundleStatus(): Promise<WisemappingBundleStatus> {
  const userIndex = join(getUserWisemappingRoot(), 'index.html')
  ...
  const resolved = resolveWisemappingIndexPath()
  if (!resolved) {
...
export function getWisemappingManifestUrl(updateUrl: string): string {
  return `${trimBase(updateUrl)}/plugins/wisemapping/manifest.json`
}
...
export async function fetchWisemappingManifest(
  updateUrl: string,
): Promise<WisemappingPluginManifest> {
  const url = getWisemappingManifestUrl(updateUrl)
  if (!url.startsWith('https://')) {
    throw new Error('Manifest URL must use HTTPS')
  }
  const m = await fetchJSON<WisemappingPluginManifest>(url)
  if (
    !m.version ||
    !m.zipUrl ||
    !m.sha256 ||
    ...
    throw new Error('Invalid WiseMapping manifest')
  }
...
function sendWisemappingProgress(payload: WisemappingInstallProgress): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send('wisemapping:install-progress', payload)
  }
}

function sendWisemappingBundleReady(): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send('wisemapping:bundle-ready')
  }
}
...
export async function installWisemappingFromManifest(
  manifest: WisemappingPluginManifest,
): Promise<void> {
  assertMinAppVersion(manifest.minAppVersion)

  const pluginsRoot = join(app.getPath('userData'), 'plugins')
  const tmpRoot = join(pluginsRoot, '.wisemapping-install-staging')
  const tmpZip = join(tmpRoot, 'bundle.zip')
  const tmpExtract = join(tmpRoot, 'extracted')

  sendWisemappingProgress({ phase: 'downloading', percent: 0, received: 0, total: 0 })
  ...
  try {
    ...
    sendWisemappingProgress({ phase: 'verifying' })
    ...
    sendWisemappingProgress({ phase: 'extracting', percent: 0 })
    ...
    const target = getUserWisemappingRoot()
    ...
    sendWisemappingProgress({ phase: 'done' })
    sendWisemappingBundleReady()
  } catch (e: unknown) {
    const message = e instanceof Error ? e.message : String(e)
    sendWisemappingProgress({ phase: 'error', message })
    throw e
  } finally {
    try {
...
export async function removeWisemappingBundle(): Promise<void> {
  const root = getUserWisemappingRoot()
  if (existsSync(root)) {
    await rm(root, { recursive: true, force: true })
  }
}

export async function installWisemappingUsingUpdateUrl(
  updateUrl: string,
): Promise<void> {
  const manifest = await fetchWisemappingManifest(updateUrl)
  await installWisemappingFromManifest(manifest)
}
